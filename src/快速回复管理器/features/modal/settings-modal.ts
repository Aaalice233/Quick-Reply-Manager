...
// 类型导入
// ============================================================================
import type { ConnectorButton, QrLlmSettings, QrLlmSecretConfig, QrLlmPresetStore, QrLlmPreset } from '../../types';
...
// 工具函数导入
// ============================================================================
import { uid, escapeHtml, getInputValueTrim, getInputBox, resolveHostWindow } from '../../utils/dom';
import { deepClone, nowIso, splitMultiValue, joinMultiValue } from '../../utils/data';
import { copyTextRobust } from '../../utils/network';

// ============================================================================
// 服务导入
// ============================================================================
import { getDebugLogText } from '../../services/debug';
import {
...
// UI 导入
// ============================================================================
import { showModal, iconSvg, toast, createCircularColorPicker } from '../../ui/components';
import { renderWorkbench } from '../../ui/workbench';
import { isClickSuppressed, runSnapshotReorderDrag } from '../../ui/events';
...
export function showSettingsModal(): void {
  ...
  showModal(
    close => {
      ...
      card.className = 'fp-settings-content';
      ...
      const normalizeImportedPreset = (
        raw: unknown,
        fallbackName = '',
      ): { name: string; preset: QrLlmPreset } | null => {
        ...
        const expandStVars = (
          ...
          const resolveVarReads = (text: string) =>
            String(text || '')
              ...
              .replace(/\{\{(setvar|addvar)::([^:}]+)::([\s\S]*?)\}\}/gi, (_all, _op, _key, body) =>
                String(body || ''),
              );
...
seed: -1`;
      ...
      const resolvePlaceholdersWithMap = (
        raw: string,
        draft: Record<string, string>,
        roleMap: Record<string, string>,
      ) => {
        let result = raw;
        Object.entries(draft).forEach(([key, defaultValue]) => {
          const roleValue = roleMap[key];
          const value = roleValue !== undefined ? roleValue : defaultValue;
      ...
      if (settingsSaveBtn)
        settingsSaveBtn.onclick = () => {
          ...
          localQrLlmSettings.generationParams.top_p = Math.max(
            ...
            Math.min(
              1,
              Number.isFinite(localQrLlmSettings.generationParams.top_p)
                ? localQrLlmSettings.generationParams.top_p
                : 1,
            ),
          );
          ...
          localQrLlmSecret.apiKey = String(
            (card.querySelector('[data-qr-api-key]') as HTMLInputElement | null)?.value ||
              localQrLlmSecret.apiKey ||
              '',
          );
    ...
    { cardClassName: 'fp-modal-card fp-settings-card' },
  );
}
